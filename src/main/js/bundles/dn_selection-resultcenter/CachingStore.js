 ...
 */
export default class CachingStore {
    /**
    ...
    constructor(args) {
        ...
        this.maxFeaturesLimit = 500;
        this.initialQuery = null;
        this.querySequenz = null;
    ...
    async _getItems(itemIds, opts) {
        const remainingIds = new Set(Array.isArray(itemIds) ? Stream.from(itemIds) : Stream.of(itemIds));
        if (!remainingIds.size) {
            return [];
        }

        /** @type {import("store-api/api").Store<any>} */
        const masterStore = this.masterStore;
        const { maxRecordCount } = await getStoreInfo(masterStore);
        const serverMax = maxRecordCount == null
            ? this.maxFeaturesLimit
            : Math.min(this.maxFeaturesLimit, maxRecordCount);

        const idChunks = (Array.isArray(itemIds) ? Stream.from(itemIds) : Stream.of(itemIds))
            .chunk(serverMax || 500)
            .toArray();

        const results = [];
        const fetchChunk = async (ids) => {
            const query = this._buildQuery(ids);
            const options = Object.assign({}, opts, {
                start: 0,
                count: ids.length,
                total: ids.length // suppress the feature count request (LayerStore)
            });

            let queryResult;
            do {
                queryResult = await masterStore.query(query, options);
                options.start += queryResult.length;
                options.count -= queryResult.length;
                results.push(...queryResult);
            } while (queryResult.hasMore);
        };

        await Promise.all(idChunks.map(ids => fetchChunk(ids)));
        return results;
    }
    ...
    _populateCache() {
        ...
        const fetchCache =
            this._fetchCache ||
            (this._fetchCache = when(
                this._getAllMasterStoreFields(),
                (allFields) => {
...
/**
 * @param {import("store-api/api").Store<any>} store
 */
async function getStoreInfo(store) {
    const metadata = await store.getMetadata?.();
    const maxRecordCount = store.maxRecordCount ?? metadata?.maxRecordCount ?? undefined;
